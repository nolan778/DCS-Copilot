...
// \param[in] tickTimeMS The time in milliseconds between main loop updates (Default = 10ms)  This time must be between [5, 30] ms.
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
StartupResult RakPeer::Startup(unsigned int maxConnections, RakNet::TimeMS tickTimeMS, SocketDescriptor *socketDescriptors, unsigned socketDescriptorCount, int threadPriority)
{
	if (IsActive())
...
RakNet::TimeMS RakPeer::GetTickTime(void) const
{
	return tickTime;
}
...
void RakPeer::OnConnectionRequest( RakPeer::RemoteSystemStruct *remoteSystem, RakNet::Time incomingTimestamp )
{
	...
	bitStream.Write(RakNet::GetTime());
	bitStream.WriteBitsFromIntegerRange((int)tickTime, 5, 30);
	bitStream.WriteBitsFromIntegerRange((int)defaultTimeoutTime, 500, 120000);
...
//								SystemAddress internalID;
								...
								inBitStream.Read(sendPongTime);
								OnConnectedPong(sendPingTime, sendPongTime, remoteSystem);
								int serverTickTimeMS, serverTimeoutTime;
								inBitStream.ReadBitsFromIntegerRange(serverTickTimeMS, 5, 30);
								inBitStream.ReadBitsFromIntegerRange(serverTimeoutTime, 500, 120000);
								tickTime = (RakNet::Time)serverTickTimeMS;
								SetTimeoutTime((RakNet::Time)serverTimeoutTime, UNASSIGNED_SYSTEM_ADDRESS);
...
// #endif
		...
		rakPeer->quitAndDataEvents.WaitOnEvent(10); //rakPeer->tickTime
