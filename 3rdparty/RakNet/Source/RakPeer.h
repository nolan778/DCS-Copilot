...
public:
	...
	StartupResult Startup(unsigned int maxConnections, RakNet::TimeMS tickTimeMS, SocketDescriptor *socketDescriptors, unsigned socketDescriptorCount, int threadPriority = -99999);
	...
	/// \brief Returns the tick time in milliseconds of the main update loop.
	/// \return Time in milliseconds between update ticks
	RakNet::TimeMS GetTickTime(void) const;
...
protected:
	...
	RakNet::TimeMS tickTime; /// Time in milliseconds between tick updates for the server (09/25/15 CSP - was default 10ms, but made it variable based on server setting)
