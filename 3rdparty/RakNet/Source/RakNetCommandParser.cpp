...
{
	RegisterCommand(4, "Startup","( unsigned int maxConnections, RakNet::TimeMS tickTimeMS, unsigned short localPort, const char *forceHostAddress );");
...
bool RakNetCommandParser::OnCommand(const char *command, unsigned numParameters, char **parameterList, TransportInterface *transport, const SystemAddress &systemAddress, const char *originalString)
{
	...
	{
		RakNet::SocketDescriptor socketDescriptor((unsigned short)atoi(parameterList[2]), parameterList[3]);
		ReturnResult(peer->Startup((unsigned short)atoi(parameterList[0]), (unsigned int)atoi(parameterList[1]), &socketDescriptor, 1), command, transport, systemAddress);
	}
